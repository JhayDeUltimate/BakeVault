...
export default function HomePage() {
  ...
  const featuredProducts = useMemo(() => featuredFromDB.map(mapDBProduct).slice(0, 4), [featuredFromDB])
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20 w-full">
        <div className="space-y-16 sm:space-y-24">
          ...
          <section className="hidden lg:block">
            <SectionHeading eyebrow="Browse Fast" title="Find ingredients by category"
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 mt-12">
              {CATEGORIES.map(category => (
                <button key={category} onClick={() => navigate('/catalog')}
          ...
          <section>
            <div className="flex items-center justify-between mb-8 sm:mb-10 border-b-2 border-orange-50 pb-6 gap-4">
              ...
              <Link to="/catalog" className="text-brand-brown font-bold text-xs sm:text-sm hover:text-brand-orange transition-colors whitespace-nowrap">Full Catalog →</Link>
            </div>

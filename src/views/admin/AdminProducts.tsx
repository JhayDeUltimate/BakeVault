...
type SortKey = 'name' | 'created_at' | 'updated_at' | 'is_available' | 'is_featured' | 'category'
type SortDir = 'asc' | 'desc'

function SortBtn({ label, field, current, dir, onClick }: { label: string; field: SortKey; current: SortKey; dir: SortDir; onClick: (f: SortKey) => void }) {
  const active = current === field
  return (
    <button type="button" onClick={() => onClick(field)}
      className={`flex items-center gap-1 text-xs font-bold uppercase tracking-wider transition-colors ${active ? 'text-orange-600' : 'text-gray-500 hover:text-gray-700'}`}>
      {label}
...
export default function AdminProducts() {
  ...
  async function handleSave(data: ProductFormData) {
    const payload = { ...data, image_urls: data.image_urls }
    if (modal?.mode === 'add')  await createProduct(payload)
    if (modal?.mode === 'edit') await updateProduct(modal.product.id, payload)
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="px-4 py-3 text-left"><SortBtn label="Product"    field="name"         current={sortKey} dir={sortDir} onClick={handleSort} /></th>
                <th className="px-4 py-3 text-left"><SortBtn label="Category"   field="category"     current={sortKey} dir={sortDir} onClick={handleSort} /></th>
                <th className="px-4 py-3 text-left"><SortBtn label="Available"  field="is_available" current={sortKey} dir={sortDir} onClick={handleSort} /></th>
                <th className="px-4 py-3 text-left"><SortBtn label="Hero"       field="is_featured"  current={sortKey} dir={sortDir} onClick={handleSort} /></th>
                <th className="px-4 py-3 text-left"><SortBtn label="Updated"    field="updated_at"   current={sortKey} dir={sortDir} onClick={handleSort} /></th>

...
const Header: React.FC<HeaderProps> = ({ cartCount, onOpenCart, onOpenCategories }) => {
  ...
  function handleSearchClick() {
    // Preserve any existing catalog query params (e.g. an active
    // category filter) when the user is already on /catalog, so
    // clicking search doesn't wipe out a filter they had set. When
    // navigating from any other page, start with a clean query string.
        ...
        <div className="flex justify-between items-center h-16 sm:h-20">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button
              ...
              className="p-2.5 text-brand-darkGray hover:text-brand-orange transition-colors bg-orange-50 rounded-full"
